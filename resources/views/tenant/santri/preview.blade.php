...
<div class="page-inner">
    ...
    <div class="page-header">
        <h3 class="fw-bold mb-3">Preview Import Santri</h3>
    </div>
        ...
        <div class="card-body">
            ...
            @if($batch->status === 'committed')
                <div class="alert alert-success">
                    Batch sudah di-commit oleh 
                    <strong>{{ optional($batch->committer)->name }}</strong> 
                    pada {{ \Carbon\Carbon::parse($batch->committed_at)->format('d M Y H:i') }}
                </div>
            @endif
                    ...
                    <tbody>
                        @foreach($batch->rows as $row)
                            <tr class="{{ !$row->is_valid ? 'table-danger' : '' }}">
                                ...
                                <td>
                                    @if(isset($row->payload['mode']))
                                        <span class="badge bg-info">
                                            {{ strtoupper($row->payload['mode']) }}
                                        </span>
                                    @endif
                                </td>
                                <td>
            ...
            @if($batch->invalid_rows == 0 && 
                ...
                <form action="{{ route('tenant.santri.import.commit', $batch->id) }}" 
                      method="POST" 
                      class="mt-3">
                    @csrf
